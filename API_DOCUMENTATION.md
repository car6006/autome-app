...
## 📋 Base Information
- **Base URL**: `https://content-capture-1.preview.emergentagent.com/api`
- **API Version**: v2.1.0
...
**Create Template**:
```bash
curl -X POST https://content-capture-1.preview.emergentagent.com/api/templates \
...
**Add Tag to Note**:
```bash
curl -X POST https://content-capture-1.preview.emergentagent.com/api/notes/abc123/tags \
  -H "Authorization: Bearer $JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"tag": "important"}'
```
...
**Search Notes by Tag**:
```bash
curl https://content-capture-1.preview.emergentagent.com/api/notes/by-tag/meeting \
  -H "Authorization: Bearer $JWT_TOKEN"
```
