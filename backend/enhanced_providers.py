...
class TranscriptionProvider:
    ...
    async def _transcribe_with_emergent(self, audio_file_path: str, session_id: str = None, chunk_idx: int = None) -> dict:
        ...
        try:
            # Note: This is a placeholder for Emergent audio transcription
            # The current emergentintegrations library focuses on chat
            # We'll need to check if audio transcription is available
            logger.info(f"🔍 Attempting Emergent transcription for {audio_file_path}")
            
            # For now, we'll return None to fall back to OpenAI
            # TODO: Implement when Emergent audio transcription is available
            return None
